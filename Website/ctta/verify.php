...
<head>
    ...
    <style>
        ...
        .hero-banner h1 { font-size: 36px; font-weight: 800; margin: 0 0 10px 0; letter-spacing: 1px; }
        .hero-banner h1 span { color: var(--primary); }
        .hero-banner p { font-size: 16px; color: #d1d5db; margin: 0; }
        
        /* CONTAINER */
        .container { max-width: 800px; margin: -30px auto 40px auto; padding: 0 20px; position: relative; z-index: 10; width: 100%;}
        .search-box { background: #fff; padding: 40px; border-radius: 12px; box-shadow: 0 10px 25px rgba(0,0,0,0.05); text-align: center; border: 1px solid #e5e7eb;}
        .search-box input { width: 100%; max-width: 400px; padding: 15px; border: 2px solid #d1d5db; border-radius: 8px; font-size: 16px; outline: none; transition: border 0.3s; margin-bottom: 20px; text-align: center; box-sizing: border-box;}
        .search-box input:focus { border-color: #ffb703; }
        .search-box button { background: #ffb703; color: #000; border: none; padding: 15px 30px; font-size: 15px; font-weight: 800; border-radius: 8px; cursor: pointer; transition: 0.2s; text-transform: uppercase;}
        .search-box button:hover { background: #fb8500; transform: translateY(-1px); }
...
<script>
    function searchMember() {
        ...
        fetch('api_eicr.php?action=verify_member&query=' + encodeURIComponent(query))
        .then(res => res.json())
        .then(data => {
