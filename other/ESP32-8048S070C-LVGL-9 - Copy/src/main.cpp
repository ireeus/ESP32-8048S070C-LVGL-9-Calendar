...
struct SettingsUI {
  lv_obj_t *location_ta;
  lv_obj_t *username_ta;
  lv_obj_t *device_id_ta;
  lv_obj_t *temp_adjust_ta;
};
// Event structure
struct Event {
  String summary;
...
void updateWeatherDisplay() {
  ...
  // Weather image (unchanged position)
  ...
  lv_obj_align(weather_img, LV_ALIGN_CENTER, 0, -75); // Adjustable
  // NEW: Temperature label moved here, positioned below the image and above the description
  ...
  lv_obj_align(temp_label, LV_ALIGN_CENTER, 0, -20); //X=horizotal , Y=vertical Adjusted Y-offset to place below image (e.g., -50; fine-tune as needed)
  ...
  lv_obj_align(aqi_label_local, LV_ALIGN_CENTER, -160, -60); // Positioned to the right, symmetric to description
  // Description (shifted down slightly to accommodate temperature)
  ...
  lv_obj_align(desc_label, LV_ALIGN_CENTER, -150, -50); // Adjusted Y-offset (e.g., from -40 to -30) to drop by 10px
  // Header container now only for location (temperature removed)
  ...
  lv_obj_align(header_cont, LV_ALIGN_CENTER, -150, -75); // Adjusted Y-offset (e.g., from -25 to -15) to drop by 10px
  ...
  lv_obj_set_style_pad_column(header_cont, 3, 0); // Decreased margin
  // Location (unchanged)
  ...
  lv_obj_align(loc_label, LV_ALIGN_CENTER, 0, -10); // Adjusted Y-offset (e.g., from -25 to -15) to drop by 10px
  // Buttons container (shifted down slightly)
  lv_obj_t *buttons_cont = lv_obj_create(weatherContainer);
  lv_obj_set_size(buttons_cont, 380, 48);
  lv_obj_align(buttons_cont, LV_ALIGN_CENTER, 0, 30); // Adjusted Y-offset (e.g., from 15 to 25) to drop by 10px
  ...
  lv_obj_set_size(hum_cont, 108, 48);
  ...
  lv_obj_set_size(wind_cont, 108, 48);
  ...
  lv_obj_set_size(precip_cont, 108, 48);
  ...
  // Forecast container (shifted down slightly)
  lv_obj_t *forecast_cont = lv_obj_create(weatherContainer);
  lv_obj_set_size(forecast_cont, 350, 80);
  lv_obj_align(forecast_cont, LV_ALIGN_CENTER, 0, 90); // Adjusted Y-offset (e.g., from 70 to 75)
  ...
  for (int i = 0; i < 7; i++) {
    ...
    lv_obj_set_style_pad_row(day_cont, -30, 0);// adjustable forecasted temp
...
void updateEventDisplay(lv_obj_t * calendar) {
  ...
  for (int i = 0; i < numEvents; i++) {
    if (events[i].isToday) {
      ...
      lv_obj_set_size(event_cont, 361, 65); // Increased height by 3 pixels
      ...
      if (summary.length() > 55) summary = summary.substring(0, 55) + "..."; // Truncate with "..."
      String description = events[i].description;
      if (description.length() > 55) description = description.substring(0, 55) + "..."; // Truncate with "..."
      String start_time_str = events[i].start.substring(11, 16);
      String end_time_str = events[i].end.substring(11, 16);
      // Title label (blue, dropped down by adjusting y-offset)
      lv_obj_t *title_label = lv_label_create(event_cont);
      lv_label_set_text(title_label, summary.c_str());
      lv_obj_set_style_text_color(title_label, lv_color_hex(0x0000FF), 0); // Blue
      lv_obj_set_style_text_font(title_label, &lv_font_montserrat_14, 0);
      lv_obj_set_style_text_align(title_label, LV_TEXT_ALIGN_LEFT, 0);
      lv_label_set_long_mode(title_label, LV_LABEL_LONG_WRAP);
      lv_obj_set_width(title_label, lv_pct(100));
      lv_obj_align(title_label, LV_ALIGN_TOP_LEFT, 0, 0); // Dropped by ~2.5% (adjusted y from 5 to 8)
      // Time row
      ...
      if (events[i].isAllDay) {
        ...
        lv_obj_set_style_text_color(from_label, lv_color_hex(0x800000), 0); // Yellow
        ...
        lv_obj_set_style_text_color(start_time_label, lv_color_hex(0x800000), 0); // Yellow
        ...
        lv_obj_set_style_text_color(to_label, lv_color_hex(0x800000), 0); // Yellow
        ...
        lv_obj_set_style_text_color(end_time_label, lv_color_hex(0x800000), 0); // Yellow
      ...
      y_offset += 75; // Adjusted for increased height
      displayed++;
    }
  }
  ...
  lv_obj_set_style_text_color(upcomingLabel, lv_color_hex(0x800000), 0);
    ...
    if (!events[i].isToday) {
      ...
      if (sscanf(startDate.c_str(), "%d-%d-%d", &year, &month, &day) == 3) {
        ...
        if (eventTime >= todayStart) {
          ...
          String start_time_str = events[i].start.substring(11, 16);
          String end_time_str = events[i].end.substring(11, 16);
          String timeText = events[i].isAllDay ? "All day" : "from " + start_time_str + " to " + end_time_str;
          // Title label (blue, dropped down)
          lv_obj_t *title_label = lv_label_create(event_cont);
          lv_label_set_text(title_label, summary.c_str());
          lv_obj_set_style_text_color(title_label, lv_color_hex(0x0000FF), 0); // Blue
          lv_obj_set_style_text_font(title_label, &lv_font_montserrat_14, 0);
          ...
          lv_label_set_long_mode(title_label, LV_LABEL_LONG_WRAP);
          lv_obj_set_width(title_label, lv_pct(100));
          lv_obj_align(title_label, LV_ALIGN_TOP_LEFT, 0, 0); // Dropped by ~2.5%
          // Date and time row (date + ", " + timeText)
          lv_obj_t *date_label = lv_label_create(event_cont);
          lv_label_set_text(date_label, (startDate + ", ").c_str());
          ...
          if (events[i].isAllDay) {
            ...
            lv_obj_set_style_text_color(all_day_label, lv_color_hex(0x800000), 0);
            lv_obj_set_style_text_font(all_day_label, &lv_font_montserrat_14, 0);
            lv_obj_align_to(all_day_label, date_label, LV_ALIGN_OUT_RIGHT_MID, 0, 0);
            time_base = all_day_label;
          } else {
            lv_obj_t *from_label = lv_label_create(event_cont);
            lv_label_set_text(from_label, "from ");
            lv_obj_set_style_text_color(from_label, lv_color_hex(0x800000), 0); // Yellow
            lv_obj_set_style_text_font(from_label, &lv_font_montserrat_14, 0);
            lv_obj_align_to(from_label, date_label, LV_ALIGN_OUT_RIGHT_MID, 0, 0);
            lv_obj_t *start_time_label = lv_label_create(event_cont);
            lv_label_set_text(start_time_label, start_time_str.c_str());
            lv_obj_set_style_text_color(start_time_label, lv_color_hex(0x800000), 0); // Yellow
            lv_obj_set_style_text_font(start_time_label, &lv_font_montserrat_14, 0);
  ...
  if (debug == 1) Serial.println("[APP] Event display updated: " + String(displayed) + " today, " + String(upcomingCount) + " upcoming");
}
void wifi_connect_cb(lv_event_t * e) {
...
void location_submit_cb(lv_event_t * e) {
  ...
  preferences.end();
  if (debug == 1) Serial.println("[APP] Location saved: " + location);
  // Geocode
  HTTPClient httpGeo;
  String geoUrl = "https://geocoding-api.open-meteo.com/v1/search?name=" + location + "&count=1&language=en&format=json";
  httpGeo.begin(geoUrl);
  ...
  if (httpCodeGeo == HTTP_CODE_OK) {
    ...
    if (!error) {
      ...
      preferences.putString("lon", lon);
      preferences.end();
      if (debug == 1) Serial.println("[APP] Geocoded to lat: " + lat + ", lon: " + lon);
    } else {
      if (debug == 1) Serial.println("[APP] Geocode JSON parsing failed");
    }
  } else {
    if (debug == 1) Serial.println("[APP] Geocode request failed");
  }
  httpGeo.end();
  if (settings_popup) {
  ...
  if (code == LV_EVENT_FOCUSED) {
    if (ta && keyboard) {
      ...
      if (is_number) {
        ...
        if (new_event_popup) lv_obj_align(new_event_popup, LV_ALIGN_BOTTOM_MID, 0, 0);
      } else {
...
void show_settings_popup() {
  ...
  lv_obj_set_size(settings_popup, 750, 500);
  ...
  // Enable vertical scrolling for the popup content.
  ...
  lv_label_set_text(version_settings_label, ("Firmware Version: " + currentFirmwareVersion).c_str());
  ...
  y_offset += 50; // Reduced spacing
  // UI color adjust (darkness slider)
  lv_obj_t *darkness_label = lv_label_create(settings_popup);
  lv_label_set_text(darkness_label, "UI:");
  lv_obj_align(darkness_label, LV_ALIGN_TOP_LEFT, 50, y_offset);
  ...
  lv_obj_set_width(darkness_slider, 300);
  lv_obj_align(darkness_slider, LV_ALIGN_TOP_MID, 0, y_offset);
  ...
  lv_obj_align(location_label, LV_ALIGN_TOP_LEFT, 50, y_offset);
  ...
  lv_obj_set_width(location_ta, 300);
  lv_obj_align(location_ta, LV_ALIGN_TOP_MID, 0, y_offset);
  lv_obj_set_style_text_font(location_ta, &lv_font_montserrat_14, 0);
  lv_obj_set_user_data(location_ta, (void*)0);
  lv_obj_add_event_cb(location_ta, keyboard_event_cb, LV_EVENT_FOCUSED, location_ta);
  lv_obj_add_event_cb(location_ta, keyboard_event_cb, LV_EVENT_DEFOCUSED, location_ta);
  y_offset += 50; // Reduced spacing
  // ParcelBox container (for username and device id only, height +100)
  lv_obj_t *parcelbox_cont = lv_obj_create(settings_popup);
  lv_obj_set_size(parcelbox_cont, 550, 200);
  ...
  lv_obj_set_style_radius(parcelbox_cont, 10, 0);
  lv_obj_set_style_pad_all(parcelbox_cont, 5, 0);
  lv_obj_set_flex_flow(parcelbox_cont, LV_FLEX_FLOW_COLUMN);
  lv_obj_set_flex_align(parcelbox_cont, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_START);
  lv_obj_set_style_pad_row(parcelbox_cont, 10, 0);
  lv_obj_set_scrollbar_mode(parcelbox_cont, LV_SCROLLBAR_MODE_OFF); // Not scrollable within the container
  // ParcelBox title label
  ...
  lv_obj_align(parcelbox_title, LV_ALIGN_TOP_MID, 0, 0);
  // Username row
  ...
  lv_obj_set_width(username_ta, 300);
  ...
  lv_obj_set_width(device_id_ta, 300);
  ...
  lv_obj_set_style_text_color(preview_label, lv_color_hex(0xFFFF00), 0); // Yellow for visibility
  // Add event callback to update preview on value change
  lv_obj_add_event_cb(device_id_ta, device_id_preview_cb, LV_EVENT_VALUE_CHANGED, preview_label);
  y_offset += 220; // Reduced spacing after container
  // Temperature adjustment
  lv_obj_t *temp_adjust_label = lv_label_create(settings_popup);
  lv_label_set_text(temp_adjust_label, "Temp Adjust:");
  lv_obj_align(temp_adjust_label, LV_ALIGN_TOP_LEFT, 50, y_offset);
  lv_obj_set_style_text_font(temp_adjust_label, &lv_font_montserrat_14, 0);
  lv_obj_set_style_text_color(temp_adjust_label, lv_color_hex(0xFFFFFF), 0);
  lv_obj_t *temp_adjust_ta = lv_textarea_create(settings_popup);
  lv_textarea_set_one_line(temp_adjust_ta, true);
  lv_textarea_set_placeholder_text(temp_adjust_ta, "0");
  char buf[10];
  snprintf(buf, sizeof(buf), "%d", temp_adjust);
  lv_textarea_set_text(temp_adjust_ta, buf);
  lv_obj_set_width(temp_adjust_ta, 100);
  lv_obj_align(temp_adjust_ta, LV_ALIGN_TOP_MID, 0, y_offset);
  ...
  lv_obj_add_event_cb(wifi_logout_btn, wifi_logout_cb, LV_EVENT_PRESSED, NULL);
  lv_obj_align(wifi_logout_btn, LV_ALIGN_TOP_MID, -100, y_offset);
  lv_obj_set_size(wifi_logout_btn, 120, 50);
  lv_obj_set_style_bg_color(wifi_logout_btn, lv_color_hex(0x333333), 0);
  lv_obj_t *wifi_logout_label = lv_label_create(wifi_logout_btn);
  lv_label_set_text(wifi_logout_label, "WiFi Logout");
  lv_obj_center(wifi_logout_label);
  ...
  lv_obj_align(api_logout_btn, LV_ALIGN_TOP_MID, 100, y_offset);
  lv_obj_set_size(api_logout_btn, 120, 50);
  ...
  // Buttons (Save and Close on bottom)
  ...
  lv_obj_align(submit_btn, LV_ALIGN_BOTTOM_LEFT, 120, 100);
  lv_obj_set_size(submit_btn, 120, 40);
  ...
  lv_obj_align(close_btn, LV_ALIGN_BOTTOM_RIGHT, -120,100);
  ...
  lv_obj_align(factory_reset_btn, LV_ALIGN_TOP_MID, 0, y_offset);
  lv_obj_set_size(factory_reset_btn, 140, 40);
  ...
  lv_obj_set_style_text_font(factory_reset_label, &lv_font_montserrat_14, 0);
}
void factory_reset_cb(lv_event_t *e) {
...
lv_color_t original_bg_color = lv_obj_get_style_bg_color(lv_scr_act(), 0);
...
// Optional: Adjust zoom if the image size needs scaling (e.g., for a 16x16 icon; use 256 for 100% scale)
lv_img_set_zoom(splash_img, 256);
lv_obj_t *loading_img = lv_img_create(lv_scr_act());
lv_img_set_src(loading_img, &loading);
lv_obj_align_to(loading_img, splash_img, LV_ALIGN_OUT_BOTTOM_MID, 0, 10);
lv_img_set_zoom(loading_img, 256);
lv_anim_t a;
lv_anim_init(&a);
lv_anim_set_var(&a, loading_img);
lv_anim_set_values(&a, 0, 3600); // 360 degrees * 10 (since rotation is in 0.1 deg units)
lv_anim_set_exec_cb(&a, (lv_anim_exec_xcb_t) lv_img_set_angle);
lv_anim_set_time(&a, 2000); // 1 second for full rotation
lv_anim_set_repeat_count(&a, LV_ANIM_REPEAT_INFINITE);
lv_anim_start(&a);
// Refresh display to show the splash immediately
loop_display();
// Add a delay to display the splash for a brief period during boot (e.g., 3 seconds; adjust as needed)
...
while (millis() - start < 3000) {
  ...
  lv_obj_align(calendar, LV_ALIGN_TOP_LEFT, 10, 60); // Dropped by 20px
  ...
  lv_style_set_text_color(&style_today, lv_color_hex(0x0000FF)); // Blue text for the date
  lv_style_set_border_width(&style_today, 2);
  lv_style_set_border_color(&style_today, lv_color_hex(0x0000FF)); // Blue frame around the button
  lv_obj_add_style(calendar, &style_today, LV_PART_ITEMS | LV_STATE_USER_1);
  // Apply text color to calendar
  lv_obj_set_style_text_color(calendar, text_color, LV_PART_ITEMS | LV_STATE_DEFAULT);
  const char * day_names[7] = {"Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"};
  lv_calendar_set_day_names(calendar, day_names);
  rearrange_calendar_parts(calendar);
  ...
  lv_obj_align_to(month_label, calendar, LV_ALIGN_OUT_TOP_MID, -150, -5);
  ...
  checkFirmwareUpdate();
  // New: Fetch and set background image if exists
  ...
  // New: Create WiFi signal strength icon next to the notification area
  wifi_icon = lv_img_create(lv_scr_act());
  lv_img_set_src(wifi_icon, getWifiImage());
  lv_img_set_zoom(wifi_icon, 109); // Adjust zoom based on icon size (e.g., for 16x16 icon)
  lv_obj_align(wifi_icon, LV_ALIGN_TOP_RIGHT, -60, -13); // Positioned to the left of the blinking box
...
lv_obj_set_style_bg_color(lv_scr_act(), original_bg_color, 0);  lv_tick_inc(5);
}
void button_event_cb(lv_event_t * e) {
...
void setup() {
  ...
  preferences.end();
  if (debug == 1) Serial.println("[APP] Loaded firmware version: " + currentFirmwareVersion);
  // Test Preferences read/write read
  preferences.begin("firmware", false);
  ...
  if (testRead == testWrite) {
    if (debug == 1) Serial.println("[APP] Preferences read/write test passed");
  } else {
    if (debug == 1) Serial.println("[APP] Preferences read/write test failed: wrote " + testWrite + ", read " + testRead);
  }
  preferences.end();
  // Load UI darkness level
  preferences.begin("ui", false);
  ...
  if (ssid == "" || password == "") {
    ...
    if (debug == 1) Serial.println("[APP] Connecting to WiFi: " + ssid);
    ...
    if (WiFi.status() == WL_CONNECTED) {
      ...
      preferences.end();
      if (apiCode == "") {
        if (debug == 1) Serial.println("[APP] No API code found, showing API code screen");
        setup_display();
        show_api_code_screen();
      } else {
      ...
      if (debug == 1) Serial.println("[APP] Stored WiFi credentials credentials failed (status: " + String(WiFi.status()) + ")");
      setup_display();
      show_wifi_setup_screen();
    }
  }
...
void loop() {
  ...
  if (currentTime - lastRefreshTime >= refreshInterval && calendar && WiFi.status() == WL_CONNECTED) {
    if (debug == 1) Serial.println("[APP] Auto-refresh triggered");
    fetchEvents();
    updateEventDisplay(calendar);
    updateMonthLabel(calendar);
  ...
  if (currentTime - lastNotificationCheck >= notificationInterval && WiFi.status() == WL_CONNECTED) {
    fetchNotifications();
    lastNotificationCheck = currentTime;
  }
  // New: Update WiFi signal strength icon
  if (currentTime - lastWifiUpdate >= wifiUpdateInterval) {
    if (wifi_icon) {
      lv_img_set_src(wifi_icon, getWifiImage());
      if (debug == 1) Serial.println("[APP] Updated WiFi icon to RSSI: " + String(WiFi.RSSI()));
    }
    lastWifiUpdate = currentTime;
  }
  // New: Fetch parcelbox credentials
  if (currentTime - lastCredentialsCheck >= credentialsInterval && WiFi.status() == WL_CONNECTED) {
    fetchParcelBoxCredentials();
    lastCredentialsCheck = currentTime;
  }
  // New independent check for 1-second date-time label refresh
  if (currentTime - lastTimeLabelUpdate >= timeLabelUpdateInterval) {
    updateDateTimeLabel();
    lastTimeLabelUpdate = currentTime;
  }
  // Existing check for notifications and other actions (kept at 1-minute interval)
  if (currentTime - lastDateTimeUpdate >= dateTimeUpdateInterval) {
    time_t now;
    time(&now);
    struct tm *nowTm = localtime(&now);
    char now_buf[32];
    strftime(now_buf, sizeof(now_buf), "%Y-%m-%d %H:%M:%S", nowTm);
    if (debug == 1) Serial.println("[DEBUG] Current local time: " + String(now_buf));
    // Debug time left for notifications
    for (int i = 0; i < numEvents; i++) {
      if (events[i].notified || events[i].remind_before == 0) continue;
      time_t next_reminder;
      if (events[i].reminder_count == 0) {
        next_reminder = events[i].start_time - events[i].remind_before * 60;
      } else if (events[i].reminder_count < 3) {
        next_reminder = events[i].last_reminder_time + 300;
      } else {
        continue;
      }
      time_t time_left = next_reminder - now;
      char buf[100];
      if (time_left > 0) {
        int days = time_left / 86400;
        int hours = (time_left % 86400) / 3600;
        int mins = (time_left % 3600) / 60;
        int secs = time_left % 60;
        snprintf(buf, sizeof(buf), "[DEBUG] Event %d (%s): %d days, %d hours, %d mins, %d secs left to next notification (total %ld seconds)", i, events[i].summary.c_str(), days, hours, mins, secs, time_left);
      } else {
        snprintf(buf, sizeof(buf), "[DEBUG] Event %d (%s): Notification overdue by %ld seconds", i, events[i].summary.c_str(), -time_left);
      }
  ...
  // New: Update background image periodically
  if (currentTime - lastBackgroundUpdate >= backgroundUpdateInterval && WiFi.status() == WL_CONNECTED) {
    fetchBackgroundFilename();
    fetchAndSetBackgroundImage();
...
void show_event_details(int index, bool isReminder) {
  ...
  lv_obj_set_size(forecast_cont, 700, 150);
  ...
  lv_obj_set_style_border_width(forecast_cont, 0, 0);
  lv_obj_set_style_pad_all(forecast_cont, 0, 0);
  lv_obj_set_style_pad_column(forecast_cont, 10, 0);
  lv_obj_set_scrollbar_mode(forecast_cont, LV_SCROLLBAR_MODE_AUTO);
  // Determine forecast days, capped at 7 days ahead
  time_t now;
  time(&now);
  struct tm now_tm;
  getLocalTime(&now_tm);
  ...
  time_t max_forecast_time = today_start + 7 * 86400; // 7 days ahead
  struct tm event_start_day = *localtime(&events[index].start_time);
  event_start_day.tm_hour = 0;
  event_start_day.tm_min = 0;
  event_start_day.tm_sec = 0;
  time_t event_start_day_start = mktime(&event_start_day);
  struct tm event_end_day = *localtime(&events[index].end_time);
  event_end_day.tm_hour = 0;
  event_end_day.tm_min = 0;
  event_end_day.tm_sec = 0;
  time_t event_end_day_start = mktime(&event_end_day);
  // Cap end day at 7 days from today
  if (event_end_day_start > max_forecast_time) {
    event_end_day_start = max_forecast_time;
  }
  // Check if event spans multiple days
  bool isMultiDay = (event_end_day_start > event_start_day_start);
  // Adjust container size for multi-day events to fit up to 7 days
  if (isMultiDay) {
    lv_obj_set_size(forecast_cont, 700, 80); // Reduced height for compact display
    lv_obj_set_style_pad_column(forecast_cont, 5, 0); // Tighter spacing
  }
  for (time_t cur_day = event_start_day_start; cur_day <= event_end_day_start; cur_day += 86400) {
    int f_index = (cur_day - today_start) / 86400;
    lv_obj_t *day_cont = lv_obj_create(forecast_cont);
    // Set size based on whether it's a multi-day event
    if (isMultiDay) {
      lv_obj_set_size(day_cont, 80, 60); // Smaller size for multi-day
    } else {
      lv_obj_set_size(day_cont, 400, LV_SIZE_CONTENT);
    }
    lv_obj_set_flex_flow(day_cont, LV_FLEX_FLOW_COLUMN);
    ...
    lv_obj_set_style_border_width(day_cont, 0, 0);
    lv_obj_set_style_pad_all(day_cont, 5, 0);
    char date_buf[20];
    ...
    if (f_index >= 0 && f_index < 14) {
      // Image
      lv_obj_t *forecast_img = lv_img_create(day_cont);
      lv_img_set_src(forecast_img, getWeatherImage(forecast[f_index].weather_code));
      // Adjust image size for multi-day events (20% smaller than original)
      if (isMultiDay) {
        lv_img_set_zoom(forecast_img, 204); // 80% of 256 (100% scale = 256 in LVGL)
      } else {
        lv_img_set_zoom(forecast_img, 256); // Original size for single-day
      }
      // Temperature
      lv_obj_t *temp_l = lv_label_create(day_cont);
      lv_label_set_text(temp_l, (String(forecast[f_index].temp_max + temp_adjust, 1) + "°C").c_str());
      lv_obj_set_style_text_font(temp_l, &lv_font_montserrat_14, 0); // Smaller font for multi-day
      lv_obj_set_style_text_color(temp_l, lv_color_hex(0x000000), 0);
      // For single-day events, include additional weather details
      if (!isMultiDay) {
        // Description
        String desc = getWeatherDescription(forecast[f_index].weather_code);
        lv_obj_t *desc_l = lv_label_create(day_cont);
        lv_label_set_text(desc_l, desc.c_str());
        lv_obj_set_style_text_font(desc_l, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(desc_l, lv_color_hex(0x000000), 0);
        // Precipitation
        lv_obj_t *precip_l = lv_label_create(day_cont);
        lv_label_set_text(precip_l, ("Precipitation: " + String(forecast[f_index].precip_sum, 1) + " mm").c_str());
        lv_obj_set_style_text_font(precip_l, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(precip_l, lv_color_hex(0x000000), 0);
        // Wind
        lv_obj_t *wind_l = lv_label_create(day_cont);
        lv_label_set_text(wind_l, ("Wind: " + String(round(forecast[f_index].wind_max)) + " km/h").c_str());
        lv_obj_set_style_text_font(wind_l, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(wind_l, lv_color_hex(0x000000), 0);
        // Humidity
        lv_obj_t *hum_l = lv_label_create(day_cont);
        lv_label_set_text(hum_l, ("Humidity: " + String(round(forecast[f_index].humidity_mean)) + "%").c_str());
        lv_obj_set_style_text_font(hum_l, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(hum_l, lv_color_hex(0x000000), 0);
        // Feel
        lv_obj_t *feel_l = lv_label_create(day_cont);
        lv_label_set_text(feel_l, ("Feels like: " + String(forecast[f_index].apparent_max + temp_adjust, 1) + "°C").c_str());
        lv_obj_set_style_text_font(feel_l, &lv_font_montserrat_14, 0);
        lv_obj_set_style_text_color(feel_l, lv_color_hex(0x000000), 0);
      }
    } else {
      lv_obj_t *no_forecast_label = lv_label_create(day_cont);
      lv_label_set_text(no_forecast_label, "No forecast");
      lv_obj_set_style_text_font(no_forecast_label, &lv_font_montserrat_14, 0);
      lv_obj_set_style_text_color(no_forecast_label, lv_color_hex(0xFFFFFF), 0);
    }
  }
...
void save_settings_cb(lv_event_t *e) {
  ...
  location = String(lv_textarea_get_text(sui->location_ta));
  ...
  preferences.end();
  if (debug == 1) Serial.println("[APP] Settings saved: Location=" + location + ", Username=" + username + ", Device ID=" + device_id);
  // Geocode
  HTTPClient httpGeo;
  String geoUrl = "https://geocoding-api.open-meteo.com/v1/search?name=" + location + "&count=1&language=en&format=json";
  httpGeo.begin(geoUrl);
...
void fetchAndSetBackgroundImage() {
  ...
  int contentLength = http.getSize();
  if (contentLength <= 0) {
    if (debug == 1) Serial.println("[APP] Invalid content length for background image");
    http.end();
    return;
  }
  uint8_t* imageBuffer = (uint8_t*)ps_malloc(contentLength);
  if (!imageBuffer) {
    if (debug == 1) Serial.println("[APP] Failed to allocate memory for background image");
    http.end();
    return;
  }
  WiFiClient *stream = http.getStreamPtr();
  ...
  while (http.connected() && totalRead < contentLength) {
    ...
    if (available) {
      size_t read = stream->readBytes(imageBuffer + totalRead, min(available, contentLength - totalRead));
      totalRead += read;
    }
    delay(1);
  }
  if (totalRead != contentLength) {
    if (debug == 1) Serial.println("[APP] Incomplete background image download");
    free(imageBuffer);
    http.end();
    return;
  }
  ...
  if (debug == 1) Serial.println("[APP] Background image set successfully");
  http.end();
  // Note: imageBuffer is not freed here as it's used by LVGL; free it if background changes or on cleanup
}