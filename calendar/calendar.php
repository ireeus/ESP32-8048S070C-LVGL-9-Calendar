...
if (!isset($_SESSION['user_id']) && isset($_COOKIE['session_lock'])) {
    ...
    try {
        $stmt = $db->prepare("SELECT user_id FROM persistent_sessions WHERE session_id = ?");
...
function parse_ical_date(string $dateStr, array $params = []): ?DateTime {
    ...
    if (isset($params['VALUE']) && $params['VALUE'] === 'DATE') {
        ...
        $dateTime = DateTime::createFromFormat('Ymd\THis', $dateStr, $dateTimeZone);
...
if (isset($_POST['action']) && $_POST['action'] === 'update_event' && isset($_SESSION['user_id'])) {
    ...
    try {
        ...
        // Check if the event is editable (not from ICS)
        $stmt = $db->prepare("SELECT source FROM events WHERE event_id = ? AND user_id = ?");
        $stmt->execute([$event_id, $_SESSION['user_id']]);
        $event = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($event && $event['source'] === 'ics') {
            echo json_encode(['success' => false, 'message' => 'Cannot update ICS imported events.']);
            exit;
        }
        // Update the event
