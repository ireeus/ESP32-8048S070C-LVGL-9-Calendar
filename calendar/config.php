...
function get_icon_class($code) {
    return match($code) {
        ...
        71 => 'img/icons/Slight_snow_fal.png',
        73 => 'img/icons/Moderate_snow_fall.png',
        75 => 'img/icons/Heavy_snow_fal.png',
...
function get_weather_data() {
    global $db;
    // Default coordinates (Rochdale)
    $lat = 53.6177;
    $lon = -2.1552;
    $location_name = 'Rochdale, UK';
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        if ($prefs) {
            if ($prefs['location_type'] === 'city' && $prefs['latitude'] && $prefs['longitude']) {
                $lat = $prefs['latitude'];
                $lon = $prefs['longitude'];
                $location_name = $prefs['city_name'];
            } elseif ($prefs['location_type'] === 'geolocation') {
                $location_name = 'Current Location';
                // Note: Actual geolocation coordinates are fetched client-side and saved to the database
            }
        }
    }
