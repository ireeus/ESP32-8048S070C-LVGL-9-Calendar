...
if (isset($_POST['save_weather_location']) && isset($_SESSION['user_id'])) {
    ...
    $latitude = $location_type === 'city' ? floatval($_POST['latitude']) : null;
    $longitude = $location_type === 'city' ? floatval($_POST['longitude']) : null;
    try {
...
if (isset($_SESSION['user_id'])) {
    $stmt = $db->prepare("SELECT COUNT(*) FROM persistent_sessions WHERE user_id = ? AND (expires_at > ? OR expires_at IS NULL)");
    $stmt->execute([$_SESSION['user_id'], date('Y-m-d H:i:s')]);
    $session_lock_enabled = $stmt->fetchColumn() > 0;
}
    ...
    <?php if (isset($_SESSION['user_id'])): ?>
        <section class="form-section">
            <div class="form-content">
                ...
                <form method="POST" class="form" id="weatherForm">
                    <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
