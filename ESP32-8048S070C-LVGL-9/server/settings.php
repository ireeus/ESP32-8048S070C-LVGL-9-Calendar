...
if (isset($_POST['save_theme']) && isset($_SESSION['user_id'])) {
    ...
    if (!array_key_exists($scheme, $THEME_SCHEMES)) {
        ...
        $theme_message = 'Theme saved - it applies to your calendar only.';
    }
}
...
<style>
    ...
    body { background:#f4f7f9 !important; color:#111827 !important; font-family:'Montserrat',sans-serif !important; }
    .menu { background:#ffffff !important; }
    .menu a, .menu-items a, .menu-items li a { color:#111827 !important; }
    .menu a:hover, .menu-items a:hover, .menu-items li a:hover { color:#fb8500 !important; }
    h1, h2, h3, h4, h5, .hero-title { color:#111827 !important; }
    .cta-btn, .btn-primary { background:#ffb703 !important; color:#111 !important; }
    .cta-btn:hover, .btn-primary:hover { background:#fb8500 !important; }
    footer { background:#111827 !important; color:#9ca3af !important; }
        ...
        .settings-tab:hover { color:var(--text-main); background:#f3f4f6; }
    ...
    <section class="form-section">
        <div class="form-content">
            ...
            <div class="settings-panel" id="panel-themes" <?php echo $active_tab !== 'themes' ? 'hidden' : ''; ?>>
                <div class="settings-grid">
                    <div class="settings-col">
                        <h2 class="text-2xl font-bold mb-4">My calendar theme</h2>
                        <p class="text-base mb-4">Changes your calendar only &mdash; other accounts are not affected.</p>
                        ...
                        <form method="POST" class="form">
                            ...
                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars($_SESSION['csrf_token']); ?>">
                            <label for="themeScheme" class="text-base">Colour scheme</label>
                            <select name="scheme" id="themeScheme" class="form-select">
                                <?php foreach ($THEME_SCHEMES as $name => $cols): ?>
                                    <option value="<?php echo htmlspecialchars($name); ?>"
                                        <?php echo $name === (string)$user_theme['scheme'] ? 'selected' : ''; ?>>
                            ...
                            <div class="flex items-center justify-center gap-3">
                                <span style="display:inline-block;width:2rem;height:2rem;border-radius:9999px;background:<?php echo $THEME_SCHEMES[(string)$user_theme['scheme']][0]; ?>;"></span>
                                <span>Accent preview</span>
                                <span id="themeDarknessValue" class="font-mono"><?php echo (int)$user_theme['darkness']; ?></span>
                            </div>
                            <button type="submit" name="save_theme" class="cta-btn">Save my theme</button>
                        </form>
                    </div>
                    ...
                    <div class="settings-col">
                        ...
                        <form method="POST" class="form">
                            ...
                            <div class="flex items-center justify-center gap-3">
                                <select name="quiet_start" class="form-select" <?php echo $is_admin ? '' : 'disabled'; ?>>
                                    <?php for ($h = 0; $h < 24; $h++): ?>
                                        <option value="<?php echo $h; ?>" <?php echo (int)$policy['quiet_start'] === $h ? 'selected' : ''; ?>><?php printf('%02d:00', $h); ?></option>
                                    <?php endfor; ?>
                                </select>
                                <span>to</span>
                                <select name="quiet_end" class="form-select" <?php echo $is_admin ? '' : 'disabled'; ?>>
                                    <?php for ($h = 0; $h < 24; $h++): ?>
                            ...
                            <?php if ($is_admin): ?>
                                <button type="submit" name="save_update_policy" class="cta-btn">Save update policy</button>
