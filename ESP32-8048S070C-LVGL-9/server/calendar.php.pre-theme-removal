...
// Handle AJAX update event
if (isset($_POST['action']) && $_POST['action'] === 'update_event' && isset($_SESSION['user_id'])) {
    ...
    try {
        ...
        $remind_before = trim($_POST['remind_before'] ?? '');
...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt = $db->prepare("SELECT * FROM events WHERE user_id = ? AND start_time LIKE ? AND start_time >= ? ORDER BY start_time ASC LIMIT 10");
    $stmt->execute([$_SESSION['user_id'], $m . '%', $now]);
...
// ---------------------------------------------------------------------------
// Per-user theme, set on settings.php -> Themes. Applies to this account's
// calendar only. Wrapped because the table is created by settings.php and may
// not exist yet on a brand-new account.
// ---------------------------------------------------------------------------
$__schemes = [
    'Blue'      => ['#2196F3', '#1976D2'], 'Green'     => ['#4CAF50', '#388E3C'],
    'Blue Grey' => ['#607D8B', '#455A64'], 'Orange'    => ['#FF9800', '#F57C00'],
    'Red'       => ['#F44336', '#D32F2F'], 'Purple'    => ['#9C27B0', '#7B1FA2'],
    'Teal'      => ['#009688', '#00796B'], 'Indigo'    => ['#3F51B5', '#303F9F'],
];
$__theme = ['scheme' => 'Blue', 'darkness' => 0];
try {
    $__stmt = $db->prepare("SELECT scheme, darkness FROM user_theme WHERE user_id = ?");
    $__stmt->execute([$_SESSION['user_id']]);
    $__row = $__stmt->fetch(PDO::FETCH_ASSOC);
    if ($__row) { $__theme = $__row; }
} catch (PDOException $e) { /* no user_theme table yet */ }
if (!array_key_exists((string)$__theme['scheme'], $__schemes)) { $__theme['scheme'] = 'Blue'; }
$__dark      = max(0, min(100, (int)$__theme['darkness']));
$__primary   = $__schemes[(string)$__theme['scheme']][0];
$__hover     = $__schemes[(string)$__theme['scheme']][1];
$__gray      = 255 - (int)round($__dark * 255 / 100);
$__is_dark   = ($__dark > 50);
$__text      = $__is_dark ? '#ECEFF1' : '#111827';
$__card      = $__is_dark ? '#1b1b1b' : '#ffffff';
$__border    = $__is_dark ? '#333333' : '#e5e7eb';
...
<head>
    ...
    <style>
        /* Design tokens - set per user from settings.php -> Themes. */
        :root {
            --primary: #ffb703;
        ...
        .code-box-mobile {
            ...
            background: var(--bg-card);
            border-radius: 0.5rem;
            width: 200px; /* Fixed width for access code box */
        }
        #eventPopup {
            display: none;
            position: fixed;
        ...
        @media (max-width: 768px) {
            ...
            .code-box-mobile {
                display: block; /* Show access code below calendar on mobile */
                margin: 1rem auto;
                width: 90%;
                max-width: 500px;
            }
            ...
            .fc-header-toolbar {
                ...
                flex-direction: column !important; /* Stack title above buttons */
            ...
            .fc-toolbar-chunk:nth-child(1) {
                order: 2 !important; /* Buttons below title */
            }
            .fc-toolbar-chunk:nth-child(2) {
                order: 1 !important; /* Title at top */
                flex-grow: 1 !important;
            }
            .fc-toolbar-chunk:nth-child(3) {
                order: 3 !important; /* View switcher at bottom */
            }
            .fc-button {
...
<style>
    ...
    .sidebar-title { font-size: 0.8rem; font-weight: 800; text-transform: uppercase; letter-spacing: .5px; color: var(--text-main); margin: 0; }
    .month-events { display: flex; flex-direction: column; gap: .45rem; }
    .month-event { background: #f3f4f6; border: 1px solid #e5e7eb; border-radius: 8px; padding: .5rem .65rem; text-align: left; }
    .me-date { font-size: .7rem; font-weight: 700; color: var(--primary-hover); text-transform: uppercase; letter-spacing: .4px; }
    .me-summary { font-size: .85rem; color: var(--text-main); font-weight: 600; word-break: break-word; }
    .month-empty { font-size: .78rem; color: #6b7280; }

    @media (min-width: 769px) {
        html, body { height: 100%; }
        body { display: flex; flex-direction: column; overflow: hidden; }
        .calendar-section { flex: 1 1 auto; min-height: 0; display: flex; flex-direction: column; padding: .5rem 1.25rem .75rem; overflow: hidden; }
        .calendar-section h1 { flex-shrink: 0; }
        .app-content { flex: 1 1 auto; min-height: 0; display: flex; width: 100%; max-width: 1500px; margin: 0 auto; gap: 1rem; }
        .left-sidebar { flex: 0 0 270px; padding: 1rem; overflow-y: auto; display: flex; flex-direction: column; gap: 1rem; }
        .calendar-container { flex: 1 1 auto; min-height: 0; display: flex; }
        #calendar { flex: 1 1 auto; min-height: 0; height: 100%; }
        footer { flex-shrink: 0; }
    }
</style>

    <!-- Per-user theme (settings.php -> Themes) -->
    <style>
        :root {
            --primary: <?php echo $__primary; ?>;
            --primary-hover: <?php echo $__hover; ?>;
            --bg-body: rgb(<?php echo $__gray; ?>,<?php echo $__gray; ?>,<?php echo $__gray; ?>);
            --bg-card: <?php echo $__card; ?>;
            --text-main: <?php echo $__text; ?>;
            --border-light: <?php echo $__border; ?>;
        }
<?php if ($__is_dark): ?>
        /* FullCalendar ships its own light stylesheet, so the grid needs telling. */
        .fc, .fc .fc-view-harness, .fc .fc-scrollgrid, .fc .fc-daygrid-day,
        .fc .fc-timegrid-slot, .fc .fc-col-header-cell, .fc .fc-list {
            background: var(--bg-card); color: var(--text-main);
        }
        .fc-theme-standard td, .fc-theme-standard th, .fc .fc-scrollgrid {
            border-color: var(--border-light);
        }
        .fc .fc-day-today { background: rgba(255,255,255,.07) !important; }
        .fc .fc-button, .fc .fc-button-primary {
            background: var(--bg-card); border-color: var(--border-light); color: var(--text-main);
        }
        .fc .fc-toolbar-title, .fc a { color: var(--text-main); }
<?php endif; ?>
    </style>
</head>
<body>
    ...
    <section class="calendar-section">
        ...
        <div class="app-content">
            <div class="left-sidebar">
                ...
                <h3 class="sidebar-title">This Month</h3>
                <div class="month-events">
                    <?php if (empty($month_events)): ?>
                        <p class="month-empty">No upcoming events this month.</p>
                    <?php else: ?>
                        <?php foreach ($month_events as $me): ?>
                            <div class="month-event">
                                <div class="me-date"><?php echo date('j M', strtotime($me['start_time'])); ?></div>
                                <div class="me-summary"><?php echo htmlspecialchars($me['summary']); ?></div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </div>
            ...
            <div class="calendar-container">
                <div id="calendar"></div>
                <div class="code-box-mobile"><?php echo htmlspecialchars($_SESSION['access_code'] ?? 'N/A'); ?></div>
            </div>
        </div>
    </section>
    ...
    <div id="eventPopup" role="dialog" aria-labelledby="popupTitle">
        ...
        <form method="POST" id="eventForm" class="space-y-4">
            ...
            <input type="text" name="remind_before" id="remindBefore" min="0" placeholder="Remind before (5m, 2h, 1d, NO)" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6" aria-label="Remind Before (minutes)">
            <div class="flex justify-end space-x-2">
                ...
                <button type="submit" name="delete_event" id="deleteButton" class="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600">Delete</button>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                select: function(info) {
                    ...
                    document.getElementById('selectedDate').value = formatLocalDate(info.start);
                    document.getElementById('summary').value = 'New Event';
                    document.getElementById('description').value = '';
                    document.getElementById('startTime').value = formatLocalDateTime(info.start);
                    document.getElementById('endTime').value = info.end ? formatLocalDateTime(info.end) : formatLocalDateTime(new Date(info.start.getTime() + 15 * 60 * 1000));
                    document.getElementById('allDay').checked = info.allDay;
                    document.getElementById('startTime').disabled = info.allDay;
                    document.getElementById('endTime').disabled = info.allDay;
                ...
                dateClick: function(info) {
                    if (calendar.view.type === 'dayGridMonth') {
                        ...
                        document.getElementById('popupTitle').textContent = 'Add Event';
                        document.getElementById('eventId').value = '';
                        document.getElementById('selectedDate').value = formatLocalDate(info.date);
                        document.getElementById('summary').value = 'New Event';
                        document.getElementById('description').value = '';
                        document.getElementById('startTime').value = formatLocalDateTime(info.date);
                        document.getElementById('endTime').value = formatLocalDateTime(new Date(info.date.getTime() + 15 * 60 * 1000));
            ...
            function updateEvent(event) {
                ...
                var data = 'action=update_event' +
                           '&event_id=' + encodeURIComponent(event.id) +
                           '&summary=' + encodeURIComponent(event.title) +
                           '&start_time=' + encodeURIComponent(formatLocalDateTime(event.start)) +
                           '&end_time=' + encodeURIComponent(event.end ? formatLocalDateTime(event.end) : formatLocalDateTime(new Date(event.start.getTime() + 15 * 60 * 1000))) +
                           '&description=' + encodeURIComponent(event.extendedProps.description || '') +
                           '&remind_before=' + encodeURIComponent(event.extendedProps.remind_before || '') +
                           '&all_day=' + encodeURIComponent(event.allDay);
                xhr.send(data);
            }
