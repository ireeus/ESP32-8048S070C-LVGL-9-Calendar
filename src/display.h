...
void setup_display()
{
  ...
  // Allocate LVGL buffer in PSRAM if available, else SRAM
  disp_draw_buf = (lv_color_t *)(psram_available ? heap_caps_malloc(bufSize * 2, MALLOC_CAP_SPIRAM) : heap_caps_malloc(bufSize * 2, MALLOC_CAP_8BIT));
  if (!disp_draw_buf)
  {
